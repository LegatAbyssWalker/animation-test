...
void Animation::updateAnimation(bool manualAnimation, bool isMoving, std::string type) {
	//If the animated entity can either change animation on command or continuous change
	//Continuous change
	if (type == "Player") { currentImage.y = pDirection; }
	if (type == "Enemy") { currentImage.y = eDirection; }
